...
export class UniversityService {
	...
	getUrl(): Observable<string> {
		...
		if (this.universitiesUrl) {
			...
			return of(this.universitiesUrl);
		}
	...
	populate(): void {
		this.getUrl()
			.switchMap(universitiesUrl => this.http.post(universitiesUrl + 'ingest', null, httpOptions))
		// this.http.post('http://server:3000/u/ingest', null, httpOptions)
			// .pipe(
				// tap(() => this.log('Populated.'))
			// )
			.subscribe(
				s => {
					console.log('University service: Populate() succeeded : s is', s);
				},
				f => {
					console.error('University service: Populate() failed : f is', f);
				}
			);
	}
	...
	getUniversities (): Observable<University[]> {
		return this.getUrl()
			...
			.switchMap(universities => of(universities.sort((a, b) => a.id - b.id)))
			.pipe(
